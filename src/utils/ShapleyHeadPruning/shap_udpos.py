...
def bernstein(sample):
    """
    Empirical Bernstein Bounds enforce Multi-Armed Bandit Sampling.
    Essentially, it contributes to stopping sampling a head once
    we can be confident enough that its contribution is positive or negative.
    """
    if len(sample) < 2:
        return -1, 1
    mean = np.mean(sample)
    variance = np.std(sample)
    delta = 0.1  # 95% confidence on upper or lower bound
    R = 1  # since worst-case scenario performance of the model is 0
...
class MaskModel(torch.nn.Module):
    """
    Augments Multi-Headed Attention with a gate
    """
    ...
    def active(self, head):
        def active_memo(head):
            """
            This function decides if a given head is going to continue to be sampled.
            """
...
def preprocess_function(examples):
    """
    This is not doing anything specific with Shapley Values
    """
...
for lang in langs:
    ...
    mask = torch.ones((1, 144)).to("mps:0")
    ...
    args = transformers.TrainingArguments(
        "tmp/", per_device_eval_batch_size=1024
    )
    trainer = Trainer(
        model=fake_model,
        args=args,
    ...
    with torch.no_grad():
        ...
        attribution = sv.attribute(
            ...
            # torch.ones((1, 144)).to("mps:0"), n_samples=3000, show_progress=True
            torch.ones((1, 144)).to("mps:0"), n_samples=30, show_progress=True
        )
    fake_model.finish()
